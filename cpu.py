...
class Opcode:
    def __init__(self, hex_value: int, mnemonic: str, bytes: int):
        self.hex_value = hex_value
        self.mnemonic = mnemonic
        self.bytes = bytes

    def __str__(self):
        return f"{self.mnemonic} ({hex(self.hex_value)}) * {self.bytes} bytes"
...
class CPUInstructions:
    ...
    def get_opcode(opcode_hex) -> Opcodes:
        ...
        for opcode in CPUInstructions.Opcodes:
            if opcode.value.hex_value == opcode_hex:
                return opcode

        return CPUInstructions.Opcodes.UNKNOWN
